...
const TeacherFinalCourseOffer = () => {
    ...
    const fetchCourses = async () => {
        if (session && semester) {
            const res = await fetch(`http://localhost:5000/api/finalCourseOffer?session=${session}&semester=${semester}`);
    ...
    const handleSubmit = async () => {
        const res = await fetch('http://localhost:5000/api/finalCourseOffer', {
            method: 'POST',
            headers: {
                    ...
                    <thead>
                        <tr>
                            <th className="w-1/4 py-2">
                                <input 
                                    type="checkbox" 
                                    onChange={(e) => setSelectedCourses(new Set(e.target.checked ? courses.map(c => c.coursecode) : []))} 
                                />
                            </th>
