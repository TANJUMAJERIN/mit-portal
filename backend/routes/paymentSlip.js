...
router.get('/paymentSlip/:roll/:semester/:session', async (req, res) => {
  ...
  const { format } = req.query; // Get the format query parameter
  ...
  try {
    ...
    const courseFees = enrollments.map(enrollment => {
      const credits = enrollment.course.credit;
      const fee = credits * 4500;
      totalFee += fee;
      return { course: enrollment.course.coursename, fee };
    });

    const paymentData = {
      student: { name: student.name, roll, session, semester },
      fees: { semesterFee, labUsageFee, courseFees, totalFee },
    };
