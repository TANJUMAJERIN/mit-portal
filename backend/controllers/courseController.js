...
// 			//const type = coursecode.startsWith("MITM") ? "Major";
// 			if (type === "Major") {
...
exports.majorCourseFetch = async (req, res) => {
    ...
    try {
        ...
        if (semester === "1st") {
            ...
            const majors = await prisma.course.findMany({
                where: {
                    coursecode: {
                        startsWith: "MITM",
                        notIn: passedCourseCodes,
...
exports.submitCourseSelection = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ error: "An error occurred while enrolling in courses." });
    }
};
